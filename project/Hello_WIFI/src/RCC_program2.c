...
void RCC_voidInitSysClock(void)
{
	...
	RCC_CR   = 0x00000081; /* Enable HSI + Triming = 0 */
	     ...
	     RCC_CFGR = 0x00000010; /* select PLL as clock system & select HSI/2 as PLL input */
		 RCC_CFGR = (RCC_CFGR & 0xFFC3FFFF) | (RCC_PLL_MUL_VAL << 18); /* clear the 4 bits of PLLMUL then add the new mul value */
	     RCC_CR   = 0x01000000; /* Enable PLL */

	#elif RCC_PLL_INPUT == RCC_PLL_IN_HSE_DIV_2
	     RCC_CFGR = 0x00030010; /* select PLL as clock system & select HSE/2 as PLL input */
		 RCC_CFGR = (RCC_CFGR & 0xFFC3FFFF) | (RCC_PLL_MUL_VAL << 18); /* clear the 4 bits of PLLMUL then add the new mul value */
	     RCC_CR   = 0x01000000; /* Enable PLL */

	#elif RCC_PLL_INPUT == RCC_PLL_IN_HSE
	     RCC_CFGR = 0x00010010; /* select PLL as clock system & select HSE as PLL input */
		 RCC_CFGR = (RCC_CFGR & 0xFFC3FFFF) | (RCC_PLL_MUL_VAL << 18); /* clear the 4 bits of PLLMUL then add the new mul value */
	     RCC_CR   = 0x01000000; /* Enable PLL */
	#else
